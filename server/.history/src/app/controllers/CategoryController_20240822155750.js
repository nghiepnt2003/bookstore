...
class CategoryController {
  ...
  async delete(req, res, next) {
    try {
      // const { id } = req.params;
      // const check = await checkDocumentExistence(Category, id);
      // if (!check.exists) {
      //   return res.status(400).json({
      //     success: false,
      //     message: check.message,
  ...
  async forceDelete(req, res, next) {
    ...
    async restore(req, res, next) {
      try {
        ...
        const restoredCourse = await Course.findById(req.params.id);
        console.log("Restored Course:", restoredCourse);
