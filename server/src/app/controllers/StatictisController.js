...
class StatictisController {
  ...
  async totalByMonth(req, res) {
    try {
      ...
      // Tìm các đơn hàng của người dùng trong khoảng thời gian của tháng đó
      const orders = await Order.find({
        user: userId,
        date: { $gte: startDate, $lte: endDate },
        status: "Successed", // Chỉ lấy đơn hàng đã thành công
      }).populate({
        path: "details", // Tham chiếu đến 'details' trong Order
        model: "OrderDetail", // Model của OrderDetail
        select: "productId productName productImage productPrice quantity", // Các trường cần lấy từ OrderDetail
      });

      // Tính toán số lượng đơn hàng và tổng số tiền
