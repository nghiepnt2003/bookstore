...
const CreateProduct = ({ onClose, onRefresh }) => {
    ...
    const handleAddProduct = async (e) => {
        ...
        // Kiểm tra các trường bắt buộc
        if (!name || !price || !pageNumber || selectedCategories.length === 0 || selectedAuthors.length === 0 || !publisher || !selectedFile) {
        ...
        selectedCategories.forEach(cat => productFormData.append('categories[]', cat._id));
        selectedAuthors.forEach(auth => productFormData.append('author[]', auth._id));
