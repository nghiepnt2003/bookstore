...
class CartController {
  ...
  const checkoutCart = async (req, res) => {
    try {
      ...
      const cart = await Cart.findOne({ user: user._id, deleted: false }).populate('items.product');
  
      if (!cart) {
        return res.status(400).json({ message: "Cart not found" });
      }
  
      const selectedItems = cart.items.filter(item => item.selectedForCheckout && !item.deleted);
      ...
      const newOrder = await Order.create({
        ...
        payment: req.body.payment, // Payment information từ client
        user: user._id
      });
