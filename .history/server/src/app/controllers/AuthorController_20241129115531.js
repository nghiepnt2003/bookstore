...
class AuthorController {
  ...
  async getAll(req, res) {
    try {
      ...
      res.status(200).json({
        success: result.authors.length > 0,
        counts: result.counts,
        authors:
          result.authors.length > 0 ? result.authors : "Cannot get authors",
      });
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
    }
  ...
  async store(req, res) {
    try {
      Cloud.single("image")(req, res, async (err) => {
        ...
        const savedAuthor = await AuthorService.store(req.body, req.file);
  ...
  async forceDelete(req, res) {
    try {
      await AuthorService.forceDelete(req.params.id);
      res
        .status(200)
  ...
  async restore(req, res) {
    try {
      const restoredAuthor = await AuthorService.restore(req.params.id);
      res.status(200).json({
        success: true,
        message: "Restored author",
        restoredAuthor,
