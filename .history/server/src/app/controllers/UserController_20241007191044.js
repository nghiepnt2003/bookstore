...
class UserController {
  ...
  generateRandomPhoneNumber = async () => {
    let phoneNumber;
    let phoneExists = true;

    // Lặp lại việc tạo số điện thoại cho đến khi số này là duy nhất
    while (phoneExists) {
      phoneNumber = `0${Math.floor(1000000000 + Math.random() * 900000000)}`; // Tạo chuỗi 10 số bắt đầu bằng 0
    ...
    try {
      ...
      if (!user) {
        const newUser = new User({
          ...
          email,
          phone: "0000000000", // Có thể lấy thêm số điện thoại từ Google hoặc để mặc định
