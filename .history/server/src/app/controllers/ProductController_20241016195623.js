...
class ProductController {
  ...
  async update(req, res, next) {
    try {
      Cloud.single("image")(req, res, async (err) => {
        ...
        // Lấy product hiện tại để kiểm tra ảnh cũ
        ...
        // Nếu có file ảnh mới trong request
        if (req.file) {
          // Xóa ảnh cũ từ Cloudinary nếu sản phẩm đã có ảnh trước đó
          if (existingProduct.image) {
            const publicId = existingProduct.image
              .split("/")
              .slice(-2)
              .join("/")
              .split(".")[0]; // Lấy publicId đầy đủ bao gồm cả folder

            // Xóa ảnh cũ trên Cloudinary
            const deleteResult = await cloudinary.uploader.destroy(publicId);
            console.log("Delete Result:", deleteResult); // Đảm bảo ảnh đã xóa
          }

          // Upload ảnh mới lên Cloudinary và lưu URL vào req.body
          const result = await cloudinary.uploader.upload(req.file.path);
          req.body.image = result.secure_url; // Lưu URL của ảnh mới vào body
        }

        // Cập nhật product với dữ liệu mới từ req.body
  ...
  //       // Xử lý upload ảnh mới nếu có file trong request
  //       if (req.file) {
  //         // Xóa ảnh cũ từ Cloudinary nếu sản phẩm đã có ảnh trước đó
  //         if (existingProduct.image) {
  //           const publicId = existingProduct.image
