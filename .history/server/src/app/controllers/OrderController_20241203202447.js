...
class OrderController {
  ...
  async getOrdersByTimes(req, res) {
    try {
      const { _id } = req.user; // Lấy userId từ middleware
      const query = req.query; // Lấy các query params

      // Gọi service xử lý
      const { orders, totalAmount, counts } =
        await orderService.getOrdersByTimes(_id, query);
