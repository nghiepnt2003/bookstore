...
class CartController {
  ...
  async updateSelectedForCheckout(req, res) {
    try {
      ...
      if (!lineItem) {
        return res.status(404).json({success:true message: "LineItem not found" });
      }
      if(lineItem.selectedForCheckout){
        return res.status(200).json({ message: "LineItem not found" });
