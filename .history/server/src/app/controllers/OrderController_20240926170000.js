  ...
  async getAllsByUser(req, res) {
    try {
      ...
      const orders = await Order.find({ user: _id })
        .populate("details") // Populate các OrderDetail nếu cần
  ...
  async getAllByAdmin(req, res) {
    try {
      ...
      let queryCommand = Order.find(formatedQueries).populate({
        path: "details",
        populate: {
          path: "productId", // Populate product chi tiết trong `details`
          select: "name price image", // Chỉ lấy những trường cần thiết trong product
        },
      });
      // .populate("details"); // Populate user và chi tiết sản phẩm trong đơn hàng
