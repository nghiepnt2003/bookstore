    ...
    try {
      ...
      if (existingLineItem) {
        ...
        if (existingLineItem.quantity < 1) {
          await LineItem.findByIdAndDelete(existingLineItem._id);
          // Loại bỏ LineItem khỏi danh sách items trong cart
          cart.items = cart.items.filter(
            (itemId) => itemId.toString() !== existingLineItem._id.toString()
          );
        } else {
