...
class OrderController {
  ...
  async checkout(req, res) {
    try {
      ...
      const { payment, shippingAddress } = req.body; // Lấy phương thức thanh toán và địa chỉ giao hàng từ yêu cầu
      ...
      if (payment === Payment.MOMO) {
        const qrCodeUrl = await generateMoMoQR("0834643661", totalPrice);
        res.status(200).json({
          success: true,
          message: "Checkout successful",
          order: newOrder,
  ...
  //[PUT] /order/confirmQRcode
  // API xác nhận quét mã QR
  async confirmQRCodeScan(req, res) {
    try {
      const { orderId } = req.body; // Lấy orderId từ request
