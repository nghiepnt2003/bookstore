...
class UserController {
  ...
  async deleteUserAddress(req, res, next) {
    try {
      ...
      // Tìm user và xóa địa chỉ tại index
      const response = await User.findByIdAndUpdate(
        _id,
        { $pull: { address: { $exists: true } } }, // Xóa địa chỉ tại index
        { new: true }
      ).select("-password -role -refreshToken");
      ...
      if (index < 0 || index >= response.address.length) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid address index" });
      }

      // Cập nhật lại mảng địa chỉ
      const updatedAddress = response.address.filter(
        (_, i) => i !== Number(index)
      );

      // Cập nhật lại địa chỉ cho user
