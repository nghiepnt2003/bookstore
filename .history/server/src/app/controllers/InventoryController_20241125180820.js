...
class InventoryController {
  ...
  async getAllInventory(req, res) {
    try {
      ...
      // Filtering
      if (queries.note) {
        formatedQueries.note = { $regex: queries.note, $options: "i" };
      }
      ...
      let queryCommand = Inventory.find(formatedQueries).populate({
        ...
        populate: {
          path: "productId", // Populate thông tin sản phẩm
          model: "Product",
          select: "name costPrice stockQuantity",
        },
      });
      ...
      const limit = +req.query.limit || process.env.LIMIT_INVENTORY || 10;
      ...
      // Lấy số lượng Inventory
      const counts = await Inventory.find(formatedQueries).countDocuments();

      res.status(200).json({
        success: response.length > 0,
        counts,
        inventories: response.length > 0 ? response : "Cannot get inventories",
      });
    } catch (error) {
