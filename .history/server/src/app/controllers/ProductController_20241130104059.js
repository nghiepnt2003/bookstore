...
class ProductController {
  ...
  async getProducts(req, res) {
    try {
      ...
      // Tách xử lý filters, pagination, sorting từ query
      const filters = {}; // Add logic to parse filters
      const pagination = {
      ...
      const { products, counts } = await productService.getProducts({
        filters,
        pagination,
        sorting,
        fields,
      ...
      res.status(200).json({ success: true, counts, products });
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
    }
  }
