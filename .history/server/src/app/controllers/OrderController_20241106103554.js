...
async function createZaloPayOrder(user, totalPrice, orderId) {
  const zalopayOrder = {
    ...
    // }),
    callback_url: "https://faf1-27-77-75-170.ngrok-free.app/callback",
  ...
  try {
    ...
    return {
      success: true,
      zalopayUrl: zalopayResponse.data,
    };
  } catch (error) {
    return {
      success: false,
  ...
  async callbackZaloPay(req, res) {
    ...
    try {
      ...
      if (reqMac !== mac) {
        ...
        result.return_message = "mac not equal"; // MAC không khớp
      } else {
        // Nếu MAC hợp lệ, xử lý thanh toán
        let dataJson = JSON.parse(dataStr); // Chuyển đổi dữ liệu thành JSON
        console.log("Received payment data:", dataJson);

        // Kiểm tra trạng thái thanh toán
        if (dataJson.return_code === 1) {
          // Thanh toán thành công
          const orderId = dataJson.app_trans_id.split("_")[1]; // Lấy ID đơn hàng từ app_trans_id
          const order = await Order.findById(orderId); // Tìm đơn hàng

          if (order) {
            // Cập nhật trạng thái đơn hàng
            order.status = "Paid"; // Thay đổi trạng thái đơn hàng thành "Paid"
            await order.save();
            result.return_code = 1;
            result.return_message = "Payment successful";
          } else {
