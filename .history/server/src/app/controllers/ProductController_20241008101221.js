...
class ProductController {
  ...
  async store(req, res) {
    try {
      ...
      // Chuyển đổi authors nếu là string
  ...
  async update(req, res, next) {
    try {
      const { id } = req.params;
      const { categories, author } = req.body;
      ...
      if (author && typeof author === "string") {
        try {
          // Nếu chuỗi là JSON hợp lệ (ví dụ: "[1, 2]")
          req.body.author = JSON.parse(author).map(Number);
        } catch (error) {
          // Nếu không phải JSON, chuyển đổi bằng cách tách chuỗi với dấu phẩy
          req.body.author = author.split(",").map(Number);
        }
      }
