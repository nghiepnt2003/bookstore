...
class CartController {
  ...
  async getCart(req, res) {
    try {
      const userId = req.user._id;
      const cart = await Cart.findOne({ user: userId }).populate({
        path: "items.product", // Đảm bảo populate được thông tin sản phẩm trong items
        model: "LineItem", // Hoặc model của items (nếu khác)
        populate: [
          {
            path: "author",
            select: "name",
      ...
      // Tính toán finalPrice cho từng sản phẩm trong cart
      const itemsWithFinalPrice = cart.items.map((item) => {
        const product = item.product; // Truy cập product đã được populate
        const finalPrice =
          product.price - (product.price * product.discount) / 100; // Tính toán finalPrice

        return {
          ...item.toObject(), // Lấy tất cả các thuộc tính của item, chuyển đổi sang object
          product, // Thêm thông tin chi tiết về product
          finalPrice, // Thêm thuộc tính finalPrice
        };
      });

      // Cập nhật lại items trong cart
