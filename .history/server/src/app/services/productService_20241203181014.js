...
class ProductService {
  ...
  async createProduct(req) {
    return new Promise((resolve, reject) => {
      Cloud.single("image")(req, {}, async (err) => {
        if (err) {
          reject({
            status: 500,
        ...
        if (
          ...
          reject({ status: 400, message: "Missing inputs" });
          return;
        }

        // Kiểm tra giá trị costPrice < price
        if (costPrice >= price) {
          reject({
            status: 400,
        ...
        // Lưu URL ảnh vào req.body nếu có file ảnh
        if (req.file && req.file.path) {
          req.body.image = req.file.path; // URL ảnh từ Cloud
        }
