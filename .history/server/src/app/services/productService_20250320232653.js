...
class ProductService {
  ...
  async getProductsWithDiscount(queries) {
    ...
    formattedQueries.discount = { $ne: null };
    formattedQueries["discount.endDate"] = { $gte: new Date() }; // Thêm điều kiện lọc
    ...
    let queryCommand = Product.find(formattedQueries)
      .populate({
        ...
        match: {
          startDate: { $lte: new Date() },
          endDate: { $gte: new Date() }, // Chỉ lấy discount còn hiệu lực
        },
        select: "discountPercentage startDate endDate",
      })
      .populate({
    ...
    const productsWithFinalPrice = await Promise.all(
      products
        .filter((product) => product !== null && product.discount) // Lọc bỏ product null & discount hết hạn
        .map(async (product) => {
          const finalPrice = await product.getFinalPrice();
          ...
          return {
            ...product.toObject(),
            finalPrice: parseFloat(finalPrice.toFixed(2)), // Thêm `finalPrice`
            timeRemaining: timeRemaining > 0 ? timeRemaining : 0, // Thời gian còn lại (nếu hết hạn thì là 0)
          };
        })
    );
