...
class OrderService {
  ...
  async getOrdersByTimeRange(userId, startTime, endTime, queryParams) {
    // Kiểm tra nếu startTime hoặc endTime không hợp lệ
    ...
    // Lấy các query parameters (ngoài startTime, endTime)
    const queries = { ...queryParams };
    const excludeFields = [
      "limit",
      "sort",
    ...
    // Thêm điều kiện thời gian và trạng thái "Successed"
    formatedQueries.date = { $gte: start, $lte: end };
    formatedQueries.status = "Successed";
    formatedQueries.user = userId; // Lọc theo userId

    // Xử lý phân trang và giới hạn
    const { limit = 10, page = 1, sort = "date" } = queryParams;
    const skip = (page - 1) * limit;

    // Tìm các đơn hàng trong khoảng thời gian và theo điều kiện trên
    const orders = await Order.find(formatedQueries)
      .limit(parseInt(limit))
      .skip(skip)
      .sort({ [sort]: -1 }); // Sắp xếp theo ngày giảm dần (có thể điều chỉnh theo yêu cầu)

    // Tổng số đơn hàng để trả về trong response
    const totalOrders = await Order.countDocuments(formatedQueries);
