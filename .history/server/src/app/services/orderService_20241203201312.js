...
class OrderService {
  ...
  async getOrdersByTimeRange(userId, queryParams) (userId, startTime, endTime, queryParams) {
    ...
    // Adjust the endTime to the last moment of the given day (23:59:59.999)
    const adjustedEnd = new Date(end.setHours(23, 59, 59, 999));
  
    // Clean up queryParams to exclude pagination and time filters
    const excludeFields = [
      "limit",
      "sort",
      "page",
    ...
    // Format query to match Mongoose operators
    let queryString = JSON.stringify(queries);
    queryString = queryString.replace(
      /\b(gte|gt|lt|lte)\b/g,
      (matchedEl) => `$${matchedEl}`
    ...
    // Add date range and "Successed" status filter
    formattedQueries.date = { $gte: start, $lte: adjustedEnd };
    formattedQueries.status = "Successed";
    formattedQueries.userId = userId; // Assuming you're filtering by user
  
    // Fetch orders from the database
    const orders = await Order.find(formattedQueries).populate({
      path: "details",
      model: "OrderDetail", // Adjust this to match your model
    });
