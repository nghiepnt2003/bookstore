...
class UserService {
  ...
  async updateUserByAdmin(_id, body, file) {
    ...
    if (file) {
      if (existingUser.image) {
        // Lấy public_id từ URL của ảnh hiện tại
        old_image_url = existingUser.image.split("/").pop().split(".")[0];
      }
    ...
    try {
      ...
      if (existingUser.image && existingUser.image !== old_image_url) {
        await cloudinary.uploader.destroy(`bookstore/${old_image_url}`);
      }
    } catch (error) {
      if (body.image) {
        const publicId = body.image.split("/").pop().split(".")[0];
        await cloudinary.uploader.destroy(`bookstore/${publicId}`);
      }
    }
