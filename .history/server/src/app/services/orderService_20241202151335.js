...
class OrderService {
  ...
  async processCheckout(user, checkoutData) {
    ...
    // Kiểm tra tồn kho, tính tổng giá và lưu chi tiết đơn hàng
    ...
    // Áp dụng giảm giá dựa trên rank của user
    ...
    if (payment === Payment.MOMO) {
      return await createMoMoOrder(user, totalPrice, uniqueOrderId);
    } else if (payment === Payment.ZALOPAY) {
      return await createZaloPayOrder(user, totalPrice, uniqueOrderId);
    }

    return { success: true, message: "Checkout successful", order: newOrder };
  }
}
const config = {
