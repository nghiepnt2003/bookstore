...
class OrderService {
  ...
  async getMoMoPaymentUrl = async (orderId, user) => {
  try {
    // 1. Lấy order từ DB
    ...
    const momoResponse = await createMoMoOrder(user, order.totalPrice, uniqueOrderId);

    return momoResponse;
  } catch (error) {
    console.error("Error in getMoMoPaymentUrl:", error.message);
    return { success: false, message: "Failed to retrieve MoMo payment URL", error: error.message };
  }
}
