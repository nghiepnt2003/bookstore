...
class ProductService {
  ...
  async suggestProducts(userId, queries) {
    try {
      ...
      if (!user) throw new Error("User not found");
      const wishListProductIds =
        user.wishList?.map((p) => p._id.toString()) || [];

      // 🔹 Lấy danh sách sản phẩm đã mua từ Order
      const orders = await Order.find({ user: userId, status: "Successed" })
        .populate("details")
        .lean();
      const purchasedProductIds = new Set(
        orders.flatMap((order) =>
          order.details.map((detail) => detail.productId.toString())
        )
      );
      ...
      // Loại bỏ sản phẩm đã mua và trong wishlist
      formattedQueries._id = { $nin: userProductIds };
      ...
      let queryCommand = Product.find({
        $or: [
          { _id: { $in: recommendedProductIds } }, // Ưu tiên sản phẩm từ CF
          formattedQueries, // Kết hợp các bộ lọc khác
        ],
      })
      ...
      const productsWithFinalPrice = await Promise.all(
        suggestedProducts.map(async (product) => {
          const finalPrice = await Product.findById(
            product._id
          ).getFinalPrice();
          let timeRemaining = null;
          if (product.discount?.endDate) {
            timeRemaining = product.discount.endDate.getTime() - Date.now();
            if (timeRemaining <= 0) timeRemaining = 0;
          }
          return {
            ...product,
