...
class UserService {
  ...
  async updateUser(_id, body) {
    ...
    // Kiểm tra và xử lý ảnh cũ nếu có ảnh mới được upload
    if (file) {
    ...
    const updatedUser = await User.findByIdAndUpdate(userId, body, {
      ...
      .populate({
        path: "wishList",
        populate: {
          path: "author publisher categories", // Các thông tin liên quan
        },
      });

    // Kiểm tra trường hợp không có gì thay đổi
    if (!updatedUser) {
      throw new Error("Failed to update user");
    }

    // Kiểm tra nếu không có wishlist hoặc các thông tin liên quan được trả về
    if (!updatedUser.wishList || updatedUser.wishList.length === 0) {
      console.warn("User's wishList is empty or not populated correctly");
    }
