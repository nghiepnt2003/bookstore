...
class InventoryService {
  async getAllInventories(filters, page, limit) {
    try {
      ...
      // Fetch inventories và populate
      const inventories = await Inventory.find(formatedFilters)
        .sort({ createdAt: -1 }) // Sắp xếp theo thời gian tạo
        .skip(skip) // Phân trang
        .limit(Number(limit)) // Giới hạn số lượng tài liệu
        .populate({
          path: "inventoryDetails.productId", // Đường dẫn để populate
          select: "name price category", // Chọn các trường từ Product cần lấy
        })
        .exec();
