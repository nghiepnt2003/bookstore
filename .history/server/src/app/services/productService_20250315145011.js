  ...
  async getById(productId) {
    try {
      const product = await Product.findOne({ _id: productId })
        ...
        }); // Populate thông tin của publisher nếu cần

      if (!product) {
        return { success: false, message: "Product not found" };
      }
      mongooseToObject(product)
       = product.mongooseToObject(); // Chuyển về object để thêm field mới
      product.finalPrice = await product.getFinalPrice(); // Gọi hàm tính toán giá
