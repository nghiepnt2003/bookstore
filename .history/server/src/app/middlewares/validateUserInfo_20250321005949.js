...
const validateUserInfo = async (req, res, next) => {
  try {
    ...
    if (!username || !email || !phone || !password) {
      return res
        .status(400)
        .json({ sucess: false, message: "Vui lòng nhập đầy đủ thông tin." });
    }
    ...
    if (!emailRegex.test(email)) {
      return res
        .status(400)
        .json({ sucess: false, message: "Email không hợp lệ." });
    }
    ...
    if (!phoneRegex.test(phone)) {
      return res
        .status(400)
        .json({ sucess: false, message: "Số điện thoại không hợp lệ." });
    }
