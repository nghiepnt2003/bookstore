...
// Lấy giá tối thiểu và tối đa từ danh sách sản phẩm (chỉ gọi 1 lần khi server khởi động)
let minPrice = 0;
let maxPrice = 0;
(async () => {
  ...
  if (priceStats.length > 0) {
    ...
    maxPrice = priceStats[0].maxPrice || 1000; // Giá trị mặc định nếu không có sản phẩm
  }
})();
 ...
 * - Thể loại: cosine similarity (trọng số 5)
 * - Tác giả: cùng thì +2 điểm
 * - Giá: chuẩn hóa và tính chênh lệch (trọng số 0.5)
 */
async function calculateSimilarity(productA, productB) {
  let similarityScore = 0;

  // 🔹 Lấy danh sách thể loại toàn cục một lần (tất cả categories từ DB)
  ...
  // 1️⃣ Tính độ tương đồng thể loại (trọng số 5)
  const categoryVectorA = createCategoryVector(productA, unionCategories);
  const categoryVectorB = createCategoryVector(productB, unionCategories);
  const categorySimilarity = cosineSimilarity(categoryVectorA, categoryVectorB);
  similarityScore += categorySimilarity * 5;

  // 2️⃣ Tính độ tương đồng tác giả (trọng số 2)
  ...
  // 3️⃣ Tính độ tương đồng giá (trọng số 0.5)
